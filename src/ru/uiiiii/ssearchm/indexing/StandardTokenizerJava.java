...
public final class StandardTokenizerJava extends Tokenizer {
  ...
  private int maxTokenLength = StandardAnalyzerJava.DEFAULT_MAX_TOKEN_LENGTH;
   ...
   */
  public StandardTokenizerJava(Version matchVersion, Reader input) {
    super(input);
    init(matchVersion);
  }
   ...
   */
  public StandardTokenizerJava(Version matchVersion, AttributeSource source, Reader input) {
    super(source, input);
    init(matchVersion);
  }
   ...
   */
  public StandardTokenizerJava(Version matchVersion, AttributeFactory factory, Reader input) {
    super(factory, input);
    init(matchVersion);
  }
  ...
  private final void init(Version matchVersion) {
    if (matchVersion.onOrAfter(Version.LUCENE_34)) {
      ...
      this.scanner = new StandardTokenizerImpl31(input);
    	...
    	throw new RuntimeException("ClassicTokenizerImpl is not public class!");
  ...
  public final boolean incrementToken() throws IOException {
    ...
    while(true) {
      ...
      if (scanner.yylength() <= maxTokenLength) {
        ...
        // remain.
        if (tokenType == StandardTokenizerJava.ACRONYM_DEP) {
          if (replaceInvalidAcronym) {
            typeAtt.setType(StandardTokenizerJava.TOKEN_TYPES[StandardTokenizerJava.HOST]);
            termAtt.setLength(termAtt.length() - 1); // remove extra '.'
          } else {
            typeAtt.setType(StandardTokenizerJava.TOKEN_TYPES[StandardTokenizerJava.ACRONYM]);
          }
        } else {
